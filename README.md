...
文件夹列表 + 后缀配置 → 递归扫描 → **按内容指纹去重**（同内容多副本合并为一条，可展开查看全部路径）→ 列表展示 → 多选批量"处理"（当前为 **mock**，集中在 `main.ts` 的 `processFiles()` 一处，接真实需求时替换它）→ 已处理文件标记持久化（重启不丢）。
...
- 数据目录 `~/.local/state/iflyrec-desktop/`：`settings.json`（目录/后缀）、`processed.json`（已处理记录，按内容指纹）、`hash-cache.json`（size+mtime 指纹缓存，避免重复哈希）。
